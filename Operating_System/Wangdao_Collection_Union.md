...
```
顺序访问：从前到后的顺序对数据进行读写，如磁带、光盘、U 盘、磁盘；

随机访问：即直接访问，可以按任意的次序对数据进行读写操作，如光盘、磁盘、U 盘；
```
...
```
操作系统的引导程序位于磁盘活动分区的引导扇区，因此必然产生于分区之后；

分区是将磁盘分为由一个或多个柱面组成的分区，每个分区的起始扇区和大小都记录在磁盘主引导记录的分区表中；

对于高级格式化，即创建文件系统，操作系统将初始文件的系统数据结构存储到磁盘上。
```

❓11. 已知某磁盘的平均转速为 r 秒/转，平均寻找时间为 T 秒，每个磁道可以存储的字节数为 N，先向该磁盘读写 b 字节的数据，采用随机寻道的方法，每道的所有扇区组成一个簇，其平均访问时间是（ ）
...
读写磁盘时，磁头首先找到磁道，然后才可以将信息从磁道里读出或写入；
读写操作完成后，刺头会继续寻找下一个磁道，完成剩余的工作。

因此在随机寻道的情况下，读写一个磁道的时间要包括寻到时间和读写磁道时间，为 T+r 秒；
由于总数据量是 b 字节，需要占用的磁道数为 b/N 个，因此总平均读写时间为 (r+T)*b/N
```

✅15. 假定磁带的记录密度为 400 字符/英寸（1 inch = 0.0254 m），每条逻辑记录为 80 字符，块间隙为 0.4 英寸，现有 3000 个逻辑记录需要存储，存储这些记录需要长度为（ ）的磁带，磁带利用率是（ ）
...
```
一个逻辑记录所占磁带长度为 80/400 = 0.2 inch；
存储 3000 条逻辑记录需要的磁带长度为 (0.2+0.4)*3000 = 1800 inches；
利用率为 0.2/(0.2+0.4) = 33.3%
```

20. 「2017真题」下列选项中，磁盘逻辑格式化程序所做的工作是（ ）
