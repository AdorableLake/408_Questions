...
```
UNIX 操作系统中，所有设备都被视为特殊文件，因为 UNIX 操作系统控制和访问外部设备的方式和访问一个文件的方式是相同的
```
...
```
打开文件操作时将该文件的 FCB 存入内存的活跃文件目录表，而不是将文件内容复制到主存，找到指定文件目录是打开文件之前的操作
```
...
```
目录文件是 FCB 的集合，一个目录中既有可能有子目录，又可能有数据文件，因此目录文件中存放的是子目录和数据文件的信息
```
...
```
文件目录项即 FCB，通常有文件基本信息、存取控制信息和实用信息组成。
基本信息包括文件物理位置；文件目录项显然不包括 FCB 的物理位置信息
```
...
```
每个磁盘块中最多有 1KB/4B = 256 个索引项，则两级索引分配方式下单个文件的最大长度为 256*256*1KB = 64MB
```
...
```
第 22 个逻辑记录存放在第 5 个物理块中

22*100/512=4···152

由于文件采用的物理结构是连接文件，因此需要从目录项所指的第一个物理块开始读取，共启动磁盘 5 次；

修改后还需要写回操作，由于写回时已获得该块的物理地址，因此只需再启动磁盘 1 次；
...
```
文件的物理结构包括连续、链式、索引三种，其中链式结构不能实现随机访问，连续结构的文件不易于扩展；
因此能够随机访问且易于扩展是索引结构的特性。
```
...
```
每个磁盘块最多有 256B/4B = 64 个索引项，则两级索引分配方式下单个文件的最大长度为 4*256B + 2*64*256B + 1*64*64*256B = 1KB + 32KB + 1024KB = 1057KB
```
...
```
I.   => 当所读文件的数据结构不在内存时，产生缺页中断，原进程进入阻塞态，直到所需数据从外存调入内存后，才将该进程唤醒；

II.  => read 系统调用进程通过陷入将 CPU 从用户态切换到核心态，从而获取操作系统提供的服务；

III. => 读一个文件，首先要用 open 系统调用打开该文件；
        open 中的参数包含文件的路径名和文件名，而 read 只需使用 open 返回的文件描述符，并不是用文件名作为参数；
        read 要求用户提供三个输入参数：
            1. 文件描述符 fd；
            2. buf 缓冲区首地址；
            3. 传送的字节数 n。
        read 的功能是试图从 fd 所指示的文件中读入 n 个字节的数据，并将它们送至指针 buf 所指示的缓冲区中。
```

✅34. 「2013真题」为支持 CD-ROM 中视频文件的快速随机播放，播放性能最好的文件数据块组织方式是（ ）
...
```
每个磁盘块最多有 1KB/4B = 256 个索引项，

10*1KB = 10KB => 直接索引项能够指引的地址项

1*256*1KB = 256KB => 一级索引项能够指引的地址项

1*256*256*1KB = 2^16KB = 64MB => 二级索引项能够指引的地址项 

单个文件的最大长度应为 10KB+256KB+64MB=
10KB = 10240B > 1234B => 访问第一个磁盘块即可；
...
```
一个文件被用户进程首次被打开，即被执行了 open 操作，会把文件的 FCB 调入内存，而不会把文件内容读到内存中，只有进程希望获取文件内容时才会读入文件内容；
```
...
连续分配的文件长度固定（即定长文件），不支持可变文件长度（连续分配的文件长度虽然也可变，但是需要大量移动数据，代价较大，相比之下不太合适）；

动态分区分配是内存管理方式，不是磁盘空间的管理方式；
```
...
不同目录下的文件名可以相同，因此在考虑系统创建最多文件数量时，只需考虑索引结点的个数，即创建文件数量上限 = 索引结点数量上限；

整个系统中最多存储 2^32 个索引结点，因此整个系统最多可以表示 2^32 个文件。
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#418)
 

## 4.2.7 File Directory | 文件目录
🌟02. 下列关于目录检索的论述中，正确的是（ ）

```
A. 由于散列法具有较快的检索速度，因此现代操作系统中都用它来代替传统的顺序检索方法
B. 在利用顺序检索法时，对树形目录应采用文件的路径名，且应从根目录开始逐级检索
C. 在利用顺序检索法时，只要路径名的一个分量名为找到，就应停止查找
D. 利用顺序检索法查找完成后，即可得到文件的物理地址
```

```
目录的查询方式有两种：顺序检索法和散列检索法，通常采用顺序检索法；

在树形目录中，为了加快文件检索速度，可设置当前目录，于是文件路径可以从当前目录开始查找；

实现用户对文件的按名存取，系统先利用用户提供的文件名形成检索路径，对目录进行检索，在顺序检索中，路径名的一个分量未找到，说明路径名中的某个目录或文件不存在，因此不需要继续检索；

在顺序检索法查找完成后，得到的是文件的逻辑地址。
```

❓07. 「2010真题」设置当前工作目录的主要目的是（ ）
...
当前目录又称工作目录，进程对各个文件的访问都相当于当前目录进行，而不需要按照绝对路径进行访问，因此加快了检索速度
```

✅08. 「2009真题」设文件 F1 的当前引用计数值为 1。先建立文件 F1 的符号链接（软链接）文件 F2，再建立文件 F1 的硬链接文件 F3，然后删除文件 F1，此时文件 F2 和文件 F3 的引用计数值分别为（ ）
...
```
建立软链接时，引用计数值直接复制（即不变）；建立硬链接时，引用计数值加 1。
删除文件时，由于删除操作对于软链接是不可见的，因此不影响文件系统，只有之后再通过软链接进行访问时，发现文件不存在，直接删除软链接；但对于硬链接来说，每一次删除操作会将引用计数值减 1，只要该值不为零，文件就不会删除。

本题中，首先建立 F2，此时引用计数值为 1；
再建立 F3 的硬链接，此时 F3 的引用计数值变为 2；
接着删除 F1，因为 F2 此时没有访问，因此引用计数值保持不变，仍未 1； F3 则将引用计数值减 1，此时为 1.
```

❓09. 「2017真题」若文件 F1 的硬链接为 F2，两个进程分别打开 F1 和 F2，获得对应的文件描述符为 FD1 和 FD2，则下列叙述中正确的是（ ）
...
I.III   => 两个进程各自维护自己的文件描述符，I 错误，III 正确（❓）
II.  => 存在多个文件名只想同一个索引结点的情况，正确
```

✅10. 「2020真题」若多个进程共享同一个文件 F，则下列叙述中，正确的是（ ）

```
A. 各进程只能用 “读” 方式打开文件 F
B. 在系统打开文件表中仅有一个表项包含 F 的属性
C. 各进程的用户打开文件表中关于 F 的表项内容相同
D. 进程关闭 F 时，系统删除 F 在系统打开表中的表项
```

```
多个进程可同时以 “读” 或 “写” 方式打开文件 F，A 错误；

整个系统只有一张系统打开文件表，即在系统打开文件表中仅有一个表项包含文件 F 的属性，B 正确；

用户进程的打开文件表关于同一个共享文件的表项内容不一样相同，比如读写指针不一致，C 错误；

进程关闭文件时，只会将文件的引用计数值减 1，当引用计数值变为 0 时，系统才会删除 F 在系统打开文件表中的表现，D 错误。
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#427)
 
## 4.3.7 File System | 文件系统
...
```
UNIX 系统采用树形目录结构，文件信息存放在索引结点中；

超级块则是用来描述文件系统的。
```

✅07. 「2015真题」文件系统用位图法表示磁盘空间的分配情况，位图存于磁盘的 32～127 号块中，每个盘块占 1024B，盘块和块内字节均从 0 开始编号。假设要释放的盘块号为 409612，则位图中要修改的位所在的盘块号和块内字节序号分别为（ ）
...
```
盘块号 = 起始块号 + [盘块号/(1024*8)] = 32 + [409612/(1024*8)] = 32+50 = 82;

块内字节号 = [(盘块号%(1024*8))/8] = 2，从 0 开始，因此为 1
```

✅08. 「2019真题」下列选项中，可用于文件系统管理空闲磁盘块的数据结构是（ ）
...
索引结点是指在许多类 UNIX 文件系统中的一种数据结构;
每个索引节点保存了文件系统中的一个文件系统对象的元信息数据，但不包括数据内容或者文件名。
```
[链接](https://www.nowcoder.com/questionTerminal/1a515be7771140df9ae688d7c0b40eb9?questionTypes=100000&page=1&onlyReference=false#:~:text=%E6%96%87%E4%BB%B6%E7%B3%BB%E7%BB%9F%E7%AE%A1%E7%90%86%E7%A9%BA%E9%97%B2,%E8%A1%A8%E3%80%81%E6%96%87%E4%BB%B6%E5%88%86%E9%85%8D%E8%A1%A8%E3%80%82)

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#437)

# Chapter 5: I/O Management
## 5.1.6 I/O Management | I/O 管理
...
共享设备又称虚拟设备，是指一段时间内允许多个进程同时访问的设备，具有可寻址以及可随机访问的特点；

分享共享设备一般通过 SPOOLing/假脱机技术，不会造成进程死锁；
```
...
引入虚拟设备是为了克服独占设备速度慢、利用率低的缺陷；

虚拟设备并不是在物理上变成共享设备（即不具备物理上的共享属性，本质上仍为独占设备），一般意义上的独占设备也只是在逻辑上被分配给了若干进程。
```
...
```
中断寄存器位于计算机主机内，不存在于设备控制器内；

I/O 地址寄存器并不存在，地址寄存器（MAR）存在于 CPU 中；

编程空间一般是由体系结构和操作系统共同决定；

控制寄存器（CR）和状态寄存器（PSW）分别用于接收上层发来的命令，并存放设备状态信号，是设备控制器与上层软件的接口；

每种设备对应的设备控制器都对应一组相应的控制命令，CPU 通过控制命令控制设备控制器
```
...
```
DMA 是一种不经过 CPU 而直接从主存存取数据的数据交换模式，它在 I/O 设备和主存之间建立了一条直接数据通路，通常用于大量、高速数据交换，如磁盘、显卡等
```

✅12. （ ）用作大量的低速或中速的 I/O 设备
...
```
字节多路通道，通常含有许多非分配性子通道，其数量可达几十到几百个；
每个通道连接一台 I/O 设备，并控制该设备的 I/O 操作；
这些子通道按 “时间片轮转” 方式共享主通道，达到各个通道循环使用主通道的目的；
各个通道每次完成其 I/O 设备的一个字节的交换（也因此称为 “字节多路通道”），然后让出主通道的使用权。

因此，只要字节多路通道扫描每个字通道的速率足够快，而连接到子通道上的设备的速率不太高时，便不至于丢失信息。
```
...
“设备控制器控制设备与内存交换数据，而设备控制器又需要请求通道为他服务”
```
...
```
绘图机和打印机属于两种不同类型的设备，系统只要按设备类型配置设备驱动程序即可，即每类设备只需一个设备驱动程序
（个人觉得存疑，因为不同厂家的设备需要不同的驱动程序）
```
...
```
系统调用是操作系统提供给用户程序的通用接口，不会因为具体设备的不同而改变。
而设备驱动程序负责执行操作系统发出的 I/O 命令，会因设备不同而不同
```

✅25. 「2017真题」系统将数据从磁盘读到内存的过程包括以下操作：
...
```
DMA 传输步骤：
1. 主机向内存写入 DMA 命令块，并向 DMA 控制器写入该命令块的地址，随后启动 I/O 设备；
2. CPU 继续其他工作，DMA 控制器直接操作内存总线，将地址放到总线上开始传输；
3. 整个传输完成后，DMA 控制器中断 CPU。
...
[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#516)

## 5.2.7 Device Independent Software | 设备独立性软件
...
05. 设从磁盘将一块数据传送到缓冲区所用的时间为 80 微秒，将缓冲区中的数据传送到用户区所用的时间为 40 微秒，CPU 处理一块数据所用的时间为 30 微秒。若有多块数据需要处理，并采用单缓冲区传送某磁盘数据，则处理一块数据所用的总时间为（ ）
...
32. 「2020真题」对于具备设备独立性的系统，下列叙述中，错误的是（ ）
...
[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#527)
...
```
缓冲池是系统的共用资源，可供多个进程共享，并且既能用于输入又能用于输出（缓冲池机制具有空队列、输入队列、输出队列三个队列，可满足同时输入、输出的要求）；
...
```
采用单缓冲区传送数据时，设备与处理机对缓冲区的操作是串行的；
当进行第 i 次读磁盘数据送至缓冲区时，系统在同时读出用户区中第 i-1 次数据进行计算；
上述两项操作可以并行，并与数据从缓冲区传送到用户区的操作串行进行；

所以系统处理一块数据所用的时间为 max(80us, 30us) + 40us = 120us.
```
...
```
I.   => 在鼠标移动时，若有高优先级的操作产生，为了记录鼠标活动的情况，必须使用缓冲技术；
II.  => 由于磁盘驱动器和目标或源 I/O 设备间的吞吐量不同，必须采用缓冲技术；
III. => 为了能使数据从用户作业空间传送到磁盘或从磁盘传送用户作业空间，必须采用缓冲技术
IV.  => 为了便于多幅图形的存取及提高性能，缓冲技术是可以采用，特别是在显示当前一幅图形又要得到下一幅图形时，应采用双缓冲技术（显卡本身的显存就是一个巨大的缓冲池）
```
...
```
SPOOLing 技术特点时将独占设备改造成共享设备（逻辑上的），其主要目的是提高系统资源/独占设备的利用率
```
...
```
SPOOLing 技术需要磁盘的支持（输入井和输出井）和内存空间（输入/输出缓冲区），而不需要外围计算机的支持
```
...
```
磁盘和内存的速度差异，决定了可以将内存经常访问的文件调入磁盘缓冲区，而从高速缓冲中复制的访问则比磁盘 I/O 操作快很多（磁盘为机械操作，闪存为芯片读取，两者都不如慢表和快表快，参见《计组》三级存储机制）
```
...
```
设备可视为特殊文件，比如在 UNIX 或类 UNIX 操作系统中，设备视为文件，因此实现了读写操作；

用户使用逻辑设备名来访问物理文件，有利于设备独立性；

通过逻辑设备名访问物理设备时，需要建立逻辑设备和物理设备之间的映射关系；

应用程序按逻辑设备名访问设备，再经驱动程序的处理来控制物理设备；若更换物理设备，只需更换驱动程序即可，无须修改应用程序（比如换了把键盘，只需重新安装一个驱动程序即可正确使用）
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Operating_System/Wangdao_Collection_Union.md#536)
