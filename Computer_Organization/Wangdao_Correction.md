...
```
ALU 是一个组合逻辑电路，因此在其运算过程中必须保持两个输入端内容不变。
又由于此 CPU 内部设计采用单总线结构，因此为了得到两个不同的操作数，ALU 的一个输入端和总线相连，另一个输入端需通过一个寄存器与总线相连。

此外，ALU 的输出端也不能直接与内部总线相连，否则其输出又会通过总线反馈到输入端，影响运算结果，因此输出端需通过一个暂存器与总线相连。
（*注：建议和第六章总线内容相联系）
```
...
```
D：单周期处理器：所有指令的指令周期为一个时钟周期的处理器 => 即 CPI=1（CPI：执行一条指令所需的时钟周期数）

B：因为每条指令的 CPI=1，要考虑比较慢的指令，所以处理器的时钟频率较低；

A： 单总线数据通路将所有寄存器的输入输出端都连在了一条公共通路上，每一个时钟内只允许一次操作，无法完成指令的所有指令；

C： 控制信号由 CU 根据指令操作码的发出的信号，对于单周期处理器来说，每个指令的执行只有一个时钟周期，而在一个时钟周期内控制信号并不会变化；
    若是多周期处理器，则指令的执行需要多个时钟周期，在每个时钟周期控制器会发出不同信号。
```
...
```
执行过程中数据所经过的路径，包括路径上的部件，都被称为数据通路。
ALU、通用寄存器、状态寄存器、Cache、MMU、浮点运算逻辑、异常和中断处理逻辑等，都是指令执行过程中数据流经的部件，也属于数据通路的一部分。
...
```
执行公用的取值微程序从主存中取出机器指令后，由机器指令的操作码字段指出各个微程序的入口地址（即初始微地址码）
```
...
```
硬布线控制器：需要结合各微操作的节拍安排，进行综合分析之后，写出逻辑表达式，再设计成逻辑电路，因此时序系统更复杂；

微程序控制器：只需按照节拍的安排，顺序执行微指令，因此时序系统更简单。
```
...
```
此类公式：若指令系统中具有 n 中机器指令，则控制存储器中的微程序数至少是 n+2 个
（一个为公共的取值指令，另一个是对应中断周期的微程序，涉及到操作系统以及计组的中断相关知识）
```
...
而若干微指令组成的序列构成微程序，微程序则完成对应指令的解释执行。

在采用微程序控制器的 CPU 中，一条指令对应一个微程序，一个微程序由若干微指令构成，一条微指令会发出很多不同的微命令。
```
...
```
CPU 控制器主要有三个部件组成：指令寄存器（IR）、程序计数器（PC）、操作控制器。

状态条件寄存器属于运算器部件，保存由算术指令和逻辑指令运行或测试的结果建立的各种条件码内容，如运算结果进位标志（CF），运算结果溢出标志（V）
```
...
```
字段直接编码法将微命令字段分成若干小字段，互斥性微命令组合在同一字段内，相容性微命令分在不同的字段内，每个字段还需要留出一个状态字，表示本字段不发出任何微命令。

本题中有 5 个互斥类微命令，分别包含 7、3、12、5、6 个微命令，需要 3、2、4、3、3 位，共 15 位
```
...
```
计算机共有 32 条指令，各个指令对应的微程序平均为 4 条，因此对应的微指令为 32*4=128 条；
公共指令有 2 条，整个系统中微指令条数共有 128+2=130 条；
需要 [log2 130]=8bit 才能够寻址到 130 条微指令。
```
...
```
主存储器 <=> 主存，在 CPU 之外，用于存储指令和数据，由 RAM 和 ROM 实现；

控制存储器：用来存放实现指令系统的所有微指令，是一种 ROM（只读存储器），机器运行时只读不写，存在于 CPU 控制器内；

CS 按照微指令地址寻址，MM 按照 PC 的地址码访问；
```
...
```
汇编程序员可见的寄存器：基址寄存器、状态/标志寄存器（PSW）、程序计数器（PC）、通用寄存器组；（但对一般程序员仍 “透明”）

其他 CPU 内部的寄存器，如：地址寄存器（MAR）、数据寄存器（MDR）、IR（指令寄存器）是不可见的；

微指令寄存器对硬件设计者可见，因此对汇编语言程序员也不可见
```
...
故障异常属于程序性异常，也称软件中断；
程序性中断貌似没这种说法；
主存故障引起的中断属于硬件中断，属于终止异常；
外中断主要指外部设备引起的中断，通常是输入输出设备；
```
...
```
缺页故障、地址越界和除数为 0 都是执行某条指令时发生的故障，需要调出操作系统内核中相应的异常处理程序来处理；

Cache 缺失由 CPU 硬件实现，无需调用出异常处理程序进行处理；
```
...
```
自陷是一种内部异常；
在 X86 中，用于程序调用的 “断点设置” 功能是通过自陷机制实现的；
执行到自陷指令时，无条件或有条件地自动掉出操作系统内核程序进行执行；
CPU 执行陷阱指令后，会自动地根据不同陷阱类型进行相应的处理，然后返回到陷阱指令的下一条指令执行；
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Computer_Organization/Wangdao_Correction.md#catalog--目录)

## 5.6.6 Instruction Pipeline | 指令流水线
✅02. 下列关于超标量流水线的描述中，不正确的是（ ）

```
A. 在一个时钟周期内一条流水线可执行一条以上的指令
B. 一条指令分为多段指令由不同的电路单元完成
C. 超标量通过内置多条流水线来同时执行多个处理器，其实质是以空间换取时间
D. 超标量流水线是指运算操作并行
```

```
超标量流水线是指一个时钟周期内一条流水线可执行一条以上的指令；
一条指令分为多段指令，由不同的电路单元完成；
...
```
A. 动态流水线是指在同一时间内，当某些段正在实现某种运算时，另一些段却正在进行另一种运算，这样对提高流水线的效率很有好处，但会是流水线控制变得很复杂
B. 动态流水线是指运算操作并行
C. 动态流水线是指指令步骤并行
D. 动态流水线是指程序步骤并行
```
...
```
动态流水线是相对于静态流水线而言的，静态流水线上下段连接方式固定，而动态流水线的连接方式可变的。
```

✅04. 流水 CPU 是由一系列称为 “段” 的处理线路组成。一个 m 段流水线稳定时的 CPU 的吞吐能力，与 m 个并行部件的 CPU 的吞吐能力相比，（ ）
...
```
m 段流水线在第 m 个时钟周期后，每个时钟周期都可与完成一条指令；
m 个并行部件在 m 个时钟周期后能完成全部的 m 条指令；
因此二者的吞吐能力等同。
```
...
```
采用流水线方式，相邻或相近的两条指令可能会因为存在某种关联，后一条指令不能按照原指令的时钟周期运行，从而使流水线断流。

引起流水线阻塞的情况有三种：1. 资源相关/结构相关；2. 数据相关；3. 控制相关；

数据旁路技术是解决数据相关的主要方法之一，其主要思想是：直接讲执行结果送到其他指令所需要的地方，使得流水线不发生停顿，因此不会引起流水线阻塞
```

🌟15. 「2016真题」在无转发机制的五段基本流水线（取指、译码/读寄存器、运算、访写回寄存器）中，下列指令序列存在数据冒险的指令对是（ ）
...
```
I2、I3 如上表所示，会发生写后读（WAR），I2 在时钟 5 时将结果写入寄存器（R5），但指令 I3 在时钟 3 时读寄存器（R5）。
因此产生数据冲突
```

✅17. 「2017真题」下列关于指令流水线数据通路的叙述中，错误的是（ ）

```
A. 包含生成控制信号的控制部件
B. 包含算术逻辑运算部件（ALU）
C. 包含通用寄存器和取指部件
D. 有组合逻辑电路和时序逻辑电路组合而成
```

```
数据在功能部件之间传送的路径被称为数据通路，包括数据通路上流经的部件，如程序计数器（PC）、ALU、通用寄存器、状态寄存器、异常和中断处理逻辑等；

数据通路由控制部件控制，控制部件根据每条指令功能的不同生成对数据通路的控制信号，因此数据通路不包含控制部件。
```

🌟19. 「2019真题」在采用 “取指、译码/取数、执行、访存、写回” 5 段流水线的处理器中，执行如下指令序列，其中 s0、s1、s2、s3、t2 表示寄存器编号。
...
| store s2, o(t2) | | | | | | | | 取指 | | | | 译码/取数 | 执行 | 访存 | 写回 |

```
由上表可知：I1 和 I3、I2 和 I3、I3 和 I4 均发生了写后读（WAR）相关，因此必须等相关前一条指令完才能执行后一条指令（即表中的 “取指” 和 “译码” 中间出现了时钟空隙）。
只有 I2 和 I4 不存在数据冲突
```

20.  「2020真题」下列给出的处理器类型，理想情况下，CPI 为 1 的是（ ）
...
## 5.7.5 Multi-CPU | 多处理器
08. 下列关于多处理器的说法中，正确的是（ ）

```
I.   一般采用偶数路 CPU
II.  NUMA 架构比 UMA 构架的运算扩展性要强
III. UMA 架构需要解决的重要问题是 Cache 一致性
...
但相较于专线结构，其实际上也降低了信息传输的并行性和传输速度。
```

✅12. 传输一幅分辨率为 640*480 像素、颜色数量为 65536 的照片（采用无压缩方式），设有效数据传输率为 56kb/s，大约需要的时间是（ ）
...
```
65536 = 2^16 => 颜色深度为 16bit
因此传输时间 T = 640*480*16/(56*1000)~87.77s
（数据传输率采用 10 进制）
``` 
...
取指令时，指令是在数据线上传输的；
操作数也是在数据线上传输；
中断类型号用以指出中断向量的地址，CPU 响应中断请求后，将中断应答信号（INTR）发挥数据总线，CPU 从数据总线上读取中断类型号后，查找中断向量表，找到相应的中断处理程序入口。因此是在数据线上传输；
握手（应答）信号属于通信联络控制信号，在控制总线上传输（参见第七章 I/O ）
```

❓22. 「2012真题」某同步总线的时钟频率为 100Mhz，宽度为 32 位，地址/数据线复用，每传输一个地址或数据占用一个时钟周期。若该总线支持突发（猝发）传输方式，则一次 “主存写” 总线事务传输 128 位数据所需要的时间至少是（ ）
...
```
总线频率为 100Mhz => 时钟周期为 10ns；
总线位宽和存储字长都是 32 位，因此每个时钟周期可传送一个 32 位存储字；
突发式传送可以连续传送地址连续的数据，因此总传送时间为：传送地址 10ns，传送 128 位数据需 40ns，共50ns
```

✅30. 「2018真题」下列选项中，可提高同步总线数据传输率的是（ ）
...
I. II. => 从上式中可知，会影响总线数据传输率
III.   => 支持突发传输方式，可使在一个总线周期内传输存储地址连续的多个数据字，因此能够提高传输效率 
IV.    => 采用地址/数据线复用技术，只能降低成本，并不能提高传输率
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Computer_Organization/Wangdao_Correction.md#catalog--目录)

## 6.2.4 Bus Transaction | 总线事务
### 选择题
✅06. 在异步总线中，传送操作（ ）
...
```
异步总线即采用异步通信方式的总线。
在异步方式下，没有公用的时钟，完全依靠传送双方相互制约的 “握手” 信号来实现定时控制。传送操作是由双方按需求分配时间的。
```
...
```
全互锁和半互锁均有请求信号和回答信号的相互制约关系；
而只有不互锁方式在主设备发出请求信号后，不必等待回答信号的到来，便自己撤销了请求信号。
因此不互锁方式速度最快。
```
...
```
A：是总线的定义；
B：同步总线是指总线通信的双方采用同一个时钟信号，但是一次总线事务不一定在一个时钟周期内完成，即时钟频率不一定等于工作频率；
C：异步总线确实采用握手的方式进行通信，每次握手的过程会完成一次通信，但是一次通信往往会交换多位而并未一位数据；
D：突发传送总线事务是指发送方仔传输完地址后，连续进行若干次数据的发送。
```

### 综合题
✅01. 在异步串行传输方式下，起始位为 1 位，数据位为 7 位，偶校验位为 1 位，停止位为 1 位，假设每秒传输 1200bit，试问有效数据传输率为多少？

```
数据位为 7 位，因此有效位为 7 位，传输过程中发送的代码位共有 1+7+1+1 =10bit

有效传输率为 1200*7/(1+7+1+1)=840b/s
```
...
D. 在独立编址的方式下，CPU 需要设置专门的输入/输出指令来访问端口
```

```
在统一编址方式下，存储单元和 I/O 设备是依靠不同的地址码而不是地址线来区分的；
在独立编址方式下，存储单元和 I/O 设备是依靠不同的指令而不是地址线来区分的；
```

✅06. 在统一编址情况下，就 I/O 设备而言，其对应的 I/O 地址说法错误的是（ ）
...
```
在统一编址情况下，指令依靠地址码区分内存和 I/O 设备，若随意在地址的任何地方编址，将会给编程造成极大混乱。
因此需要提前相对固定在地址的某部分，可以是高端，也可以是低端
```
...
```
磁盘驱动器向盘片磁道记录数据时采用串行方式写入（可查阅第三章存储器磁盘章节）
```
...
```
逻辑地址：内部编程使用，不唯一； => 逻辑段管理内容（可查阅第三章存储器的段页相关章节）
物理地址：外部连接使用且唯一，与地址总线相对应； => 主存地址
```

✅09. 「2012真题」下列选项中，在 I/O 总线的数据线上传输的信息包括（ ）
...
```
I/O 总线包含三类：数据线、地址线、控制线

数据线：传输数据缓冲寄存器和命令/状态寄存器（也称 “控制/状态寄存器” ）的内容；
地址线：传送与 CPU 交换数据的端口地址；
控制线：给 I/O 端口发送读/写信号，只用来对端口进行读/写控制。

I,II,III 分别对应以上的 数据线、数据线和控制线的内容
```

✅12. 「2021真题」下列选项中，不属于 I/O 接口的是（ ）
...
磁盘驱动器即磁盘本身，不属于 I/O 接口；
打印机适配器、网络控制器、可编程中断控制器均属于 I/O 接口
```

[返回标题行](https://github.com/AdorableLake/408_Questions/blob/main/Computer_Organization/Wangdao_Correction.md#catalog--目录)

## 7.3.4 I/O Control Methods | I/O 方式
✅02. 中断判优逻辑和总线仲裁方式相类似，下列说法中正确的是（ ）

```
I.   在总线仲裁方式中，独立请求方式响应时间最快，是以增加控制线数为代价的
II.  在总线仲裁方式中，计数器定时查询方式有一根总线请求（BR）和一根设备地址线，若每次计数都是从 0 开始，则设备号小的优先级高
...
```
I.   => 独立请求方式每个 I/O 接口都有各自的总线请求和总线同意线，共 2n 根控制线以获得高速响应；
II.  => 在计数器定时方式下，n 个 I/O 接口需要 [log2n] 根设备地址线；
III. => 总线仲裁方式是总线被征用的判优方式，从设备一般是 I/O 设备，或者是外存；
IV.  => 中断判优逻辑可通过硬件实现，也可通过软件实现
```

✅04. 当有中断源发出请求时，CPU 可执行相应的中断服务程序，可以提出中断的有（ ）
...
```
I.   => 外部事件如键盘输入属于外中断
II.  => Cache 完全是由硬件实现的，不会涉及到中断层面
III. => 虚拟存储器失效，如缺页，会发出缺页中断，属于内中断
IV.  => 浮点数下溢，直接当做机器零处理，而不会引发中断（结合前面第二章数据的运算章节）
V.   => 浮点数上溢，表示超过了浮点数的表示范围，属于内中断（同样结合第二章数据的运算章节，注意与下溢区别）
```
...
```
I.   => 程序中断过程是由硬件执行的中断隐指令和中断服务程序共同完成的；
II.  => 每条指令周期结束后，CPU 会同意扫描各个中断源，然后进行判优来决定响应哪个中断源，而不是在每条指令的执行过程中这样做
III. => CPU 会在每个存储周期结束后检查是否有 DMA 请求，而不是在指令执行过程的末尾这样做
IV.  => 中断服务程序的最后指令通常是中断返回指令，与无条件转移指令不同的是，中断返回指令不仅要修改 PC（程序计数器）的值，而且要将 CPU 中的所有寄存器都恢复到中断前的状态（即恢复现场）
```
...
即使当前设备是高速设备会需要与主机批量交换数据，若没有 DMA 接口，无法产生 DMA 请求
```
...
中断优先级：访管->程序性->重新启动
重启 != 关机
重启应当等待其他任务完成后再进行，因此优先级最低（这点在日常使用中可以看出来，执行重启时，需要等待其他进程全部关闭后开始进行）
访管指令最紧迫，优先级最高（访管指令使得操作系统从用户态转至核心态，优先级较高，参见操作系统）

硬件故障优先级最高，访管指令优先级高于外部中断
```
...
```
用户程序需要输入/输出时，需要调用操作系统提供的接口（即系统调用），此时调用访管指令，将操作系统从用户态转至核心态，因此是访管中断（参见操作系统第一章）
```
...
```
选项中只有中断隐指令不属于 “程序控制指令”，因为它属于硬件直接控制指令，在响应中断时直接由硬件控制执行
```

✅16. 在中断响应周期中，由（ ）将允许中断触发器置 0
...
```
允许中断触发器置 0 表示关中断，这个操作在前期由中断隐指令自动完成（中断隐指令真的好辛苦XD）
```

```
小结：
中断响应周期由 “中断隐指令” 和 “中断服务程序” 两者共同完成。
其中中断隐指令由硬件自动完成，主要包含三个处理流程：
1. 关中断（开始介入）；
2. 保存断点（保存有效信息，主要是 PC（X86，MIPS/ARM） 和 PSW（X86） 的内容）；
3. 中断服务程序开始寻址；（中断隐指令结束工作，接下来交给中断服务程序）

中断服务程序一般包含六个处理流程：
1. 保存现场和屏蔽字；
2. 开中断（用以实现中断嵌套）；
3. 执行中断服务程序（）；
4. 关中断（为接下来的恢复现场和断点作准备）；
5. 恢复现场和屏蔽字；
6. 开中断，中断返回（结束响应周期，最后一条指令为中断返回指令，并不是无条件/条件转移指令）
```
...
```
中断屏蔽标志（屏蔽字）的其中一种作用是实现中断升级，即实现中断处理的次序改变，因此可改变多个中断服务程序执行完的次序
（区别中断响应次序，中断响应次序是由硬件排队电路决定的）
```
...
```
程序查询方式中，CPU 与外设串行工作，传送与主程序串行工作；

中断方式中，CPU 与外设并行工作，数据准备好时仍需中断主程序以执行数据传送，因此传送与主程序仍为串行工作；

DMA 方式中，结合了中断方式的优点，因此 CPU 与外设并行工作；同时传送过程无需 CPU 的介入，因此传送和主程序也是并行工作（这也是为什么高速大批量传送设备常用 DMA 方式的原因）；
```

✅24. CPU 响应 DMA 请求的条件是当前（ ）执行完
...
```
每个机器周期结束后，CPU 就可以响应 DMA 请求。

而 DMA 在与主存交互数据时则是通过周期窃取的方式，此时窃取的是主存的存取周期。
（可以理解为什么主存频率越高，核显性能越强了，因为此时存取周期很短，核心显卡的性能发挥越好）
```
...
```
前面已经回答过，PC 的内容的保护和更新由硬件自动完成（中断隐指令）
```

✅29. 在 DMA 方式传送数据的过程中，由于没有破坏（ ）的内容，所以 CPU 可以正常工作（访存除外）
...
```
DMA 方式传送数据时，周期窃取不会改变 CPU 现场，因此无需占用 CPU 的 PC（程序计数器）和 PSW（程序状态字寄存器）
```

✅31. 「2009真题」下列选项中，能引起外部中断的事件是（ ）
...
✅32. 「2010真题」单级中断系统中，中断服务程序内的执行顺序是（ ）
...
✅35. 「2012真题」响应外部中断的过程中，中断隐指令完成的操作，除保护断点外，还包括（ ）
...
补充：保存通用寄存器的内容是中断服务程序首先进行的操作
```

✅38. 「2015真题」在采用中断 I/O 方式控制打印输出的情况下，CPU 和打印控制接口中的 I/O 端口之间交换的信息不可能是（ ）
...
```
I/O 方式中，CPU 和打印机（外设）直接交换，打印字符（这里已经是二进制码了···）直接传输到打印机的 I/O 端口，不会涉及主存地址；

CPU 和打印机通过 I/O 端口中的状态口和控制口来实现交互。
```

✅40. 「2018真题」下列关于外部 I/O 中断的叙述中，正确的是（ ）

```
A. 中断控制器按所接受中断请求的先后次序进行中断优先级排队
B. CPU 响应中断时，通过执行中断隐指令完成通用寄存器的保护
C. CPU 只有处于中断允许状态时，才能响应外部设备的中断请求
D. 有中断请求时，CPU 立即暂停当前指令执行，转去执行中断服务程序
```

```
A：中断优先级由屏蔽字而非请求的先后次序决定；

B：参见上述 35，通用寄存器的保护由中断服务程序完成；

C：中断允许状态即开中断，才能响应中断请求；

D：有中断请求时，若处于关中断状态时，或中断请求优先级较低，CPU 不响应新的中断请求；
```

41. 「2019真题」某设备以中断方式与 CPU 进行数据交换，CPU 主频为 1GHz，设备接口中的数据缓冲寄存器为 32 位，设备的数据传输率为 50kB/s。若每次中断开销（包括中断响应和中断处理）为 1000 个时钟周期，则 CPU 用于该设备输入/输出的时间占整个 CPU 时间的百分比最多是（ ）
...
网络数据包到达时 CPU 指令以外的事件，属于外部中断。
```

❓45. 「2020真题」若设备采用周期挪用 DMA 方式进行输入和输出，每次 DMA 传送的数据块大小为 512 字节，相应的 I/O 接口中有一个 32 位数据缓冲寄存器，对于数据输入过程，下列叙述中，错误的是（ ）
...
```
周期挪用法由 DMA 控制器挪用一个或几个主存周期来访问主存，传送完一个数据字后立即释放总线，是一种单字传送方式，每个字传送完后，CPU 可以访问主存；
（所以是可以访问主存的）

停止 CPU 访存法则：指在整个数据块的传送过程中，使 CPU 脱离数据总线，停止访问主存
```

❓46. 「2021真题」下列是关于多重中断系统中 CPU 响应中断的叙述，错误的是（ ）
...
```
A：中断服务程序在内核态执行，在用户态下，无法检测和响应中断（参见操作系统）

B：在多重中断系统中，CPU 确实在检测到中断请求信号后，才会进入中断响应周期；

C：进入中断响应周期时，说明 CPU 一定处于中断允许即开中断状态，否则无法响应该中断（多重中断系统，若为单重中断系统，则是关中断状态）；

D：若所有中断源请求信号都被屏蔽，则 CPU 无法检测到任何中断请求信号。
```
